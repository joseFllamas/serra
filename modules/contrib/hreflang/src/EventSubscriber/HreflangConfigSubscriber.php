...
class HreflangConfigSubscriber implements EventSubscriberInterface {
  ...
  public function onSave(ConfigCrudEvent $event) {
    ...
    if (!$event->isChanged('x_default') && !$event->isChanged('defer_to_content_translation')) {
      return;
    }
    if (!$this->cacheDynamicPageCache && !$this->cachePage) {
      return;
    }
    $this->messenger->addStatus($this->t('Page caches are being cleared for the new settings to take effect.'));
    $listener = function () {
