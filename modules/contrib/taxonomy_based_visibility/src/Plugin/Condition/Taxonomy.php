...
class Taxonomy extends ConditionPluginBase {
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    foreach($vids as $vid) {
      ...
      //open grid if checkboxes are checked
      $grid_open = FALSE;
      ...
      $form[$vid->id() ] = ['#type' => 'details', '#title' => $title, '#open' => $grid_open, ];
      $form[$vid->id() ][$vid->id() ] = array(
        '#type' => 'checkboxes',
        '#title' => $title,
        '#description' => $this->t('Check the box for any taxonomy that should have this block visible') ,
        '#options' => $this->get_taxonomy($vid->id()) ,
  ...
  public function evaluate() {
    ...
    foreach($node_taxonomys_field_values as $key1 => $value1) {
  ...
  /**
   * Function To Get List of Taxonomys based on vocabulary ID
   * Params $vid=number
   */
  public function get_taxonomy($vid) {
